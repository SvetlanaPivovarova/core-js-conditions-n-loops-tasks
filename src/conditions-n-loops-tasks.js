...
function convertToRomanNumerals(num) {
  ...
  if (num >= 5) return num === 9 ? 'IX' : `V${convertToRomanNumerals(num - 5)}`;
  if (num > 0) return num === 4 ? 'IV' : `I${convertToRomanNumerals(num - 1)}`;
  return '';
}
...
function convertNumberToString(numberStr) {
  ...
  const convert = (i = 0) => {
    result += getConvertedStr(numberStr[i]);
    if (i === numberStr.length - 1) return result;
    result += ' ';
    return convert(i + 1);
  };
...
function sortByAsc(arr) {
  ...
  for (let i = 0; i < sorted.length; i += 1) initialArr[i] = sorted[i];
